<?php
	...
	define('SECURITY_PHRASE', 'rl1hP43B2xBzbESxNsUnRbvuvmgq3B');
	define('PASSWORD', '');
	...
	function sfpg_javascript()
	{
			...
			function keyNavigate(key)
			{
				if (!selecting)
				{
					...
					else
					{
						...
						{
							document.location=phpSelf+'?sfpg='+navInfo[navInfo.length-3]['navLink'];
							return false;
						}
					}
				}
		...
		{
			window.location=phpSelf+'?sfpg='+((type=='nav')?navInfo[id]['navLink']:dirInfo[id]['dirLink']);
		}
		...
		function admUpload(show)
		{
			...
			if (show)
			{
				...
				'<form action=\"'+phpSelf+'?sfpg='+dirInfo[0]['dirLink']+'\" method=\"post\" multipart=\"\" enctype=\"multipart/form-data\">'+
